'use client';

import { useEffect, useId, useState } from 'react';
import { Copy, Check } from 'lucide-react';

function truncateEthAddress(addr: string): string {
  const a = addr.trim();
  if (a.length <= 10) return a;
  return `${a.slice(0, 4)}******${a.slice(-4)}`;
}

/** MetaMask Portfolio–style card preview (SVG + balance from extension sync). */
export function MetaMaskCardWidget({
  balanceAmount,
  balanceCurrency,
  address,
}: {
  balanceAmount: string | null;
  balanceCurrency: string | null;
  address: string | null;
}) {
  const uid = useId().replace(/[^a-zA-Z0-9]/g, '');
  const maskClip = `mmc_clip_${uid}`;
  const maskWord = `mmc_mask_word_${uid}`;

  const addrDisplay = address ? truncateEthAddress(address) : '—';
  const showAmt = balanceAmount ?? null;
  const showCur = balanceCurrency?.trim() ?? '';

  const [copied, setCopied] = useState(false);

  const handleCopyAddress = () => {
    if (!address) return;
    navigator.clipboard.writeText(address).then(
      () => setCopied(true),
      (err) => console.error('Clipboard write failed:', err),
    );
  };

  useEffect(() => {
    if (!copied) return;
    const timer = setTimeout(() => setCopied(false), 1500);
    return () => clearTimeout(timer);
  }, [copied]);

  return (
    <div className="relative mx-auto w-full max-w-[min(100%,22rem)]">
      {address ? (
        <div className="mb-2 flex items-center justify-end gap-1.5">
          <span className="text-xs text-muted-foreground tabular-nums">{addrDisplay}</span>
          <button
            type="button"
            onClick={handleCopyAddress}
            className="flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
            aria-label={copied ? 'Copied!' : 'Copy wallet address'}
          >
            {copied ? (
              <>
                <Check className="h-3 w-3 text-green-500" />
                <span className="text-green-500">Copied!</span>
              </>
            ) : (
              <Copy className="h-3 w-3" />
            )}
          </button>
        </div>
      ) : null}
      <div className="relative aspect-[343/215] w-full">
        <svg
          viewBox="0 0 343 215"
          className="h-full w-full overflow-visible rounded-2xl shadow-lg"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          aria-hidden
        >
          <defs>
            <mask id={maskClip} maskUnits="userSpaceOnUse" style={{ maskType: 'luminance' }}>
              <path
                d="M12.8217 0H330.178C337.275 0 343 5.6537 343 12.6611V202.339C343 209.346 337.275 215 330.178 215H12.8217C5.7254 215 0 209.346 0 202.339V12.6611C0 5.6537 5.7254 0 12.8217 0Z"
                fill="white"
              />
            </mask>
          </defs>
          <g mask={`url(#${maskClip})`}>
            <path
              d="M12.8217 -0.000976562H330.178C337.275 -0.000976562 343 5.65273 343 12.6601V202.338C343 209.345 337.275 214.999 330.178 214.999H12.8217C5.7254 214.999 0 209.345 0 202.338V12.6601C0 5.65273 5.7254 -0.000976562 12.8217 -0.000976562Z"
              fill="#FF5C16"
            />
            <path d="M137 123H234V39H137V123Z" fill="#FFA680" />
            <path d="M188 215H273V-6H188V215Z" fill="#FFA680" />
            <path d="M272 215H332V168H272V215Z" fill="#FFA680" />
            <path d="M328 215H343V168H328V215Z" fill="#FFA680" />
            <path d="M272 168H343V77.1525L272 48V168Z" fill="#FFA680" />
            <path d="M137.047 38.978L188.092 -6.37109L217 87L137.047 38.978Z" fill="#FFA680" />
            <path d="M137.047 122.947L188.092 168.296L217 87.5L137.047 122.947Z" fill="#FFA680" />
            <path d="M280.343 137.998L252.885 129.955L232.201 142.139H217.767L197.043 129.955L169.625 137.998L161.279 110.247L169.625 79.4303L161.279 53.3914L169.625 21.1016L212.485 46.3442H237.483L280.343 21.1016L288.689 53.3914L280.343 79.4303L288.689 110.247L280.343 137.998Z" fill="#FF5C16" />
            <path d="M169.625 21.1016L212.485 46.3442L210.792 63.6636L169.625 21.1016Z" fill="#FF5C16" />
            <path d="M197.042 110.247L215.912 124.421L197.042 129.955V110.247Z" fill="#FF5C16" />
            <path d="M214.42 86.8355L210.792 63.7031L187.608 79.43L187.688 95.6346L197.083 86.8355H214.42Z" fill="#FF5C16" />
            <path d="M280.343 21.1016L237.483 46.3442L239.177 63.6636L280.343 21.1016Z" fill="#FF5C16" />
            <path d="M252.926 110.247L234.056 124.421L252.926 129.955V110.247Z" fill="#FF5C16" />
            <path d="M262.401 79.43L239.177 63.7031L235.548 86.8355H252.885L262.32 95.6346L262.401 79.43Z" fill="#FF5C16" />
            <path d="M197.043 129.955L169.625 137.998L161.279 110.247H197.043V129.955Z" fill="#E34807" />
            <path d="M214.42 86.835L219.662 120.279L212.404 101.686L187.648 95.634L197.083 86.835H214.42Z" fill="#E34807" />
            <path d="M252.926 129.955L280.343 137.998L288.689 110.247H252.926V129.955Z" fill="#E34807" />
            <path d="M235.548 86.835L230.306 120.279L237.564 101.686L262.32 95.634L252.885 86.835H235.548Z" fill="#E34807" />
            <path d="M161.279 110.246L169.625 79.4297H187.567L187.648 95.6343L212.404 101.686L219.662 120.28L215.912 124.381L197.043 110.207L161.279 110.246Z" fill="#FF8D5D" />
            <path d="M288.689 110.246L280.343 79.4297H262.401L262.32 95.6343L237.564 101.686L230.306 120.28L234.056 124.381L252.926 110.207L288.689 110.246Z" fill="#FF8D5D" />
            <path d="M237.483 46.3438H224.984H212.485L210.792 63.6632L219.662 120.28H230.347L239.217 63.6632L237.483 46.3438Z" fill="#FF8D5D" />
            <path d="M169.625 21.1016L161.279 53.3914L169.625 79.4303H187.567L210.792 63.7034L169.625 21.1016Z" fill="#661800" />
            <path d="M209.219 93.5645H201.074L196.639 97.8246L212.364 101.687L209.219 93.5645Z" fill="#661800" />
            <path d="M280.343 21.1016L288.689 53.3914L280.343 79.4303H262.401L239.177 63.7034L280.343 21.1016Z" fill="#661800" />
            <path d="M240.749 93.5645H248.894L253.329 97.8645L237.604 101.726L240.749 93.5645Z" fill="#661800" />
            <path d="M232.201 131.069L234.056 124.38L230.306 120.279H219.622L215.872 124.38L217.727 131.069" fill="#661800" />
            <path d="M232.201 131.069V142.138H217.767V131.069H232.201Z" fill="#C0C4CD" />
            <path d="M197.042 129.915L217.767 142.098V131.03L215.912 124.341L197.042 129.915Z" fill="#E7EBF6" />
            <path d="M252.926 129.915L232.201 142.138V131.07L234.056 124.381L252.926 129.915Z" fill="#E7EBF6" />
          </g>
          <mask id={maskWord} maskUnits="userSpaceOnUse" style={{ maskType: 'luminance' }}>
            <path
              d="M12.8217 0H330.178C337.275 0 343 5.6537 343 12.6611V202.339C343 209.346 337.275 215 330.178 215H12.8217C5.7254 215 0 209.346 0 202.339V12.6611C0 5.6537 5.7254 0 12.8217 0Z"
              fill="white"
            />
          </mask>
          <g mask={`url(#${maskWord})`}>
            <path
              d="M57.3277 43.3139V51.0778H53.2151V45.7028L48.538 46.2204C47.53 46.3399 47.0462 46.6584 47.0462 47.2556C47.0462 48.1315 47.8929 48.5297 49.7476 48.5297C50.8766 48.5297 52.1265 48.3704 53.2151 48.0917L51.0782 51.038C50.2315 51.2371 49.3847 51.3167 48.4977 51.3167C44.7883 51.3167 42.6513 49.8834 42.6513 47.2954C42.6513 45.026 44.3045 43.8315 48.0945 43.4334L53.0942 42.876C52.8119 41.4426 51.7233 40.8454 49.546 40.8454C47.4897 40.8454 45.2318 41.363 43.2158 42.3186L43.8609 38.8547C45.756 38.0982 47.8929 37.7001 50.0702 37.7001C54.8682 37.6204 57.3277 39.5713 57.3277 43.3139ZM20.4755 33.5593L16 51.0778H20.4755L22.6931 42.2788L26.5638 46.8176H31.2409L35.1116 42.2788L37.3291 51.0778H41.8046L37.2888 33.5593L28.9023 43.3538L20.4755 33.5593ZM37.2888 16.001L28.9023 25.8352L20.4755 16.001L16 33.5593H20.4755L22.6931 24.7602L26.5638 29.2991H31.2409L35.1116 24.7602L37.2888 33.5593H41.7643L37.2888 16.001ZM68.0931 43.1149L64.4643 42.5973C63.5773 42.438 63.2144 42.1593 63.2144 41.6815C63.2144 40.8852 64.1014 40.5269 65.9158 40.5269C68.0125 40.5269 69.9075 40.9649 71.8832 41.8408L71.3993 38.4167C69.7865 37.8593 67.9721 37.5806 66.0771 37.5806C61.6419 37.5806 59.1824 39.0936 59.1824 41.8408C59.1824 43.951 60.513 45.1454 63.3354 45.5834L67.0045 46.101C67.9318 46.2602 68.2947 46.5788 68.2947 47.1362C68.2947 47.9325 67.448 48.2908 65.7142 48.2908C63.416 48.2908 60.9565 47.7732 58.9405 46.7778L59.3437 50.2019C61.0775 50.8389 63.3354 51.1973 65.4723 51.1973C70.0284 51.1973 72.4073 49.6445 72.4073 46.8575C72.367 44.7473 71.0364 43.5528 68.0931 43.1149ZM73.9798 35.1121V51.0778H78.0924V35.1121H73.9798ZM82.8501 43.9112L88.5352 37.8593H83.4549L78.0521 44.1102L83.7775 51.0778H88.9787L82.8501 43.9112ZM73.4153 29.7769C73.4153 32.3649 75.5522 33.7982 79.2617 33.7982C80.1487 33.7982 80.9954 33.7186 81.8421 33.5195L83.9791 30.5732C82.8904 30.8519 81.6405 31.0112 80.5116 31.0112C78.6972 31.0112 77.8102 30.6528 77.8102 29.7371C77.8102 29.1399 78.2537 28.8213 79.302 28.7019L83.9791 28.1843V33.5593H88.0917V25.7556C88.0917 22.013 85.6322 20.0621 80.8745 20.0621C78.6972 20.0621 76.5602 20.4602 74.6652 21.2167L74.0201 24.6806C76.0361 23.7251 78.294 23.2075 80.3503 23.2075C82.5276 23.2075 83.6162 23.8445 83.8984 25.238L78.8988 25.7954C75.0684 26.313 73.4153 27.5075 73.4153 29.7769ZM61.9645 28.9408C61.9645 32.1658 63.8595 33.7982 67.6496 33.7982C69.1414 33.7982 70.3913 33.5593 71.5606 33.0417L72.0848 29.4982C70.9558 30.1751 69.7865 30.5334 68.6173 30.5334C66.8432 30.5334 66.0771 29.8167 66.0771 28.2639V23.5658H72.2864V20.301H66.0771V17.5538L58.2551 21.6149V23.5658H61.9645V28.9408ZM57.9325 27.4676V28.2639H46.8446C47.3284 29.8963 48.8203 30.613 51.4814 30.613C53.578 30.613 55.5133 30.175 57.2874 29.3788L56.8036 32.8028C55.1908 33.4399 53.1748 33.7982 51.1185 33.7982C45.635 33.7982 42.611 31.4093 42.611 26.9899C42.611 22.5704 45.6753 20.1019 50.3927 20.1019C55.0295 20.1019 57.9325 22.7695 57.9325 27.4676ZM46.8043 25.5167H53.8199C53.457 23.9639 52.2474 23.1676 50.3121 23.1676C48.3364 23.1676 47.1672 23.9639 46.8043 25.5167Z"
              fill="#661800"
            />
          </g>
          <text x="16" y="173" fontSize={32} fontWeight={700} fill="#262833" textAnchor="start">
            {showAmt ? (
              <>
                <tspan>{showAmt}</tspan>
                {showCur ? (
                  <tspan dx={4} fontSize={16} fontWeight={400} fill="#262833">
                    {showCur}
                  </tspan>
                ) : null}
              </>
            ) : (
              <tspan>—</tspan>
            )}
          </text>
          <text x="16" y="199" fontSize={12} fill="#262833" opacity={0.7}>
            {addrDisplay}
          </text>
          <path
            d="M276.571 152.431H304.392C317.979 152.431 328.987 163.3 328.987 176.718C328.987 190.135 317.979 201.005 304.392 201.005H276.571C262.983 201.005 251.976 190.135 251.976 176.718C251.976 163.3 262.983 152.431 276.571 152.431Z"
            fill="white"
            stroke="white"
            strokeWidth={10}
          />
          <path d="M300.319 159.199H280.563V194.236H300.319V159.199Z" fill="#FF5F00" />
          <path d="M281.813 176.717C281.813 169.909 284.998 163.419 290.401 159.199C280.603 151.594 266.37 153.266 258.669 162.981C251.008 172.656 252.702 186.631 262.54 194.236C270.765 200.606 282.216 200.606 290.441 194.236C285.038 190.015 281.813 183.525 281.813 176.717Z" fill="#EB001B" />
          <path d="M326.971 176.717C326.971 189.02 316.891 199.013 304.392 199.013C299.311 199.013 294.433 197.341 290.481 194.236C300.279 186.631 301.972 172.616 294.231 162.901C293.102 161.548 291.852 160.234 290.481 159.199C300.279 151.594 314.512 153.266 322.132 162.981C325.277 166.843 326.971 171.7 326.971 176.717Z" fill="#F79E1B" />
        </svg>
      </div>
    </div>
  );
}
